 ...
 */
  ...
  public static Iterable<Entity> listEntities(String kind, String searchBy,
  	...
  	if (searchFor != null && !"".equals(searchFor)) {
  		Query.Filter filter = new Query.FilterPredicate(searchBy, FilterOperator.EQUAL, searchFor);
  		q.setFilter(filter);
  	}
  ...
  public static Iterable<Entity> listChildren(String kind, Key ancestor) {
  	...
  	Query.Filter filter = new Query.FilterPredicate(Entity.KEY_RESERVED_PROPERTY, FilterOperator.GREATER_THAN, ancestor);
  	q.setFilter(filter);
  	PreparedQuery pq = datastore.prepare(q);
  	return pq.asIterable();
  }
